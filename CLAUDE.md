...
set -a; . ./env; set +a                      # untracked; holds the OAuth client ids
...
- **`ccd/client_config.py`** -- OAuth client identifiers *and* service configuration. **Neither provider's client is embedded**, and neither should be re-embedded: a committed client id is unrotatable without a release, ties every deployment to one application identity, and for Google trips GitHub push protection. `require(provider)` fails loudly instead. `_DEFAULT_MS_AUTHORITY` is the one Microsoft value that does have a default, because it is Microsoft's own endpoint rather than the operator's. Local values live in the untracked `env` file (`.gitignore: /env`). `base_url()` normalises `CCD_BASE_URL`, **which may carry a path prefix** (`https://voice.example.com/ccd`) because the service is normally mounted under another module's hostname with the proxy stripping the prefix. It is the only place outbound URLs are built. `ensure_api_key()` returns `(key, created)`, generating one into a `0600` `api_key` file on first start.
